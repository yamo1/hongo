...
Bus::Bus(const std::shared_ptr<Agent> &_agent, int _routeId)
    : agent(_agent), routeId(_routeId) {}

void Bus::insertTimetable(double depTime,
...
void Bus::setLowerRL(int simTime) {
  nextStationNum++;
...
void Bus::updateAcceleration(double timestep) {
  if (stopStep > 0) {
    return;
  }
...
void Bus::updateVelocity(double timestep) {
  if (stopStep > 0) {
    return;
  }
...
std::shared_ptr<Agent> Bus::moveBus(int timestep, double simTime) {
